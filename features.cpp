...
std::pair<std::string, FLinearColor> find_valid_target_text(AActor* actor) {
    ...
    if (actor->GetName().find("SkillChargeItem") != std::string::npos) return std::make_pair(std::string("Rig Recharge"), FLinearColor{ 1.f, 1.f, 0.f, 1.f });

    if (actor->GetName().find("ReviveSyringe") != std::string::npos) return std::make_pair(std::string("Revive Syringe"), FLinearColor{ 0.f, 1.f, 0.f, 1.f });
    if (actor->GetName().find("PsychosisAntidote") != std::string::npos) return std::make_pair(std::string("Antidote"), FLinearColor{ 0.f, 1.f, 0.f, 1.f });

    if (actor->GetName().find("TempHealthGain-Consumable") != std::string::npos) return std::make_pair(std::string("Small Medicine"), FLinearColor{ 0.f, 1.f, 0.f, 1.f });
...
void NpcESP(AActor* actor, APlayerController* controller, UCanvas* canvas) {
    ...
    if (controller->ProjectWorldLocationToScreen(foot_location, foot_pos, false) && controller->ProjectWorldLocationToScreen(head_location, head_pos, false)) {
        ...
        RenderBox(canvas, top, bottom, is_visible ? FLinearColor{0.f, 1.f, 0.f, 1.f} : FLinearColor{ 1.f, 0.0f, 0.0f, 1.f }, 1.f);
    }
    FVector2D w2s_result;
    if (controller->ProjectWorldLocationToScreen(location, w2s_result, false)) {
        RenderText(canvas, w2s_result, ENPCTypeToString(((ARBNPC*)actor)->NPCType), { 1.f, 1.f, 1.f, 1.f });
    }
}
...
void PlayerESP(AActor* actor, APlayerController* controller, UCanvas* canvas) {
    ...
    if (controller->ProjectWorldLocationToScreen(foot_location, foot_pos, false) && controller->ProjectWorldLocationToScreen(head_location, head_pos, false)) {
        ...
        FVector2D top = { head_pos.x - w * 0.25f, head_pos.y };
        FVector2D bottom = { head_pos.x + w * 0.25f, foot_pos.y };
        RenderBox(canvas, top, bottom, is_visible ? FLinearColor{0.f, 0.f, 1.f, 1.f} : FLinearColor{ 0.f, 0.0f, 1.0f, 1.f }, 1.f);
    }
    auto name = ((ARBPlayer*)actor)->GetGamePlayerName();
    if (name.is_valid()) {
        FVector2D w2s_result;
        if (controller->ProjectWorldLocationToScreen(location, w2s_result, false)) {
            RenderText(canvas, w2s_result, name.to_string(), { 1.f, 1.f, 1.f, 1.f });
        }
    }
}
